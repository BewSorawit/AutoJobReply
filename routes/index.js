...
router.post('/submit-application', upload.fields([
    ...
    try {
        ...
        const { name, lastName, email, phoneNumber, fromWhen, toWhen, jobPosition } = req.body;
        const resumeFile = req.files['resume'] ? req.files['resume'][0] : null;
        const optionalFile = req.files['optionalFile'] ? req.files['optionalFile'][0] : null;
        ...
        const applicant = new Applicant({
            ...
            optionalFilePath: optionalFile ? path.join(userDir, optionalFile.filename) : null,
        });
        ...
        if (optionalFile) {
            fs.renameSync(optionalFile.path, path.join(userDir, optionalFile.filename));
        }
